...
- [ ] Ge feedback som strukturerade gapkoder och `next_action`, inte bara fri prosa. Kräv ny observation, nytt test eller förändrad plan före retry; identisk körning stoppas `no_progress`.
  ...
  - **`no_progress` gäller nu varje körning, inte bara begränsade.** #29 la regeln bakom en budget därför att den var en kostnadsregel; det här är en kontraktsregel, och identiskt underlag besvarat med identisk återkoppling är lika improduktivt i en obudgeterad körning. `metadata.no_progress.material` säger om underlaget också stod stilla. Obudgeterade körningar behåller sin 1.0-semantik i övrigt — det här ändrar *när* en körning stoppar, inte hur den betygsätts. Testet som höll den gamla gränsen synlig är omskrivet, inte borttaget.
