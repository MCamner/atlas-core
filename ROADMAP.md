...
>
> Exporten: körningsdokumentet bär `evidence_manifest`, en sanerad `atlas-observation-manifest.v1`, aldrig den råa basen. `snapshot.root` utelämnas i stället för att maskeras, eftersom sanering känner igen hemkataloger och nyckelformer men inte en godtycklig absolut sökväg. **Kvar öppet:** prosakanalen `observations` exporteras fortfarande ordagrant som i 1.0 — en adapter som lägger filinnehåll där exporterar det.
>
> Vad PR F **inte** täcker: ett sunt citat betyder fortfarande inte `verified` — det betyder att påståendet är *behörigt* till en semantisk kontroll som inte finns. Ett falskt påstående med korrekt citerad källa passerar alltså grinden, och ett test i sviten hävdar just det så att ingen läser en grön körning som verifierad. En körning utan evidensbas graderas exakt som i 1.0, och `evidence_base: null` plus tomt `citation_checks` i körningsdokumentet är det som gör skillnaden läsbar.
>
> Vad #18 (PR D) täcker: `Finding.v1` med schema; deterministisk kontroll mot körningens **observation**, inte mot filen ensam — att `source_id` finns i körningen, att den påstådda digesten matchar, att källan fortfarande är färsk, att observationens eget utdrag fortfarande matchar det radintervall det anger, och att citatet sitter sammanhängande *inom* det intervall observationen faktiskt registrerade. Allt avgörs ur bytes från en enda läsning, som vägrar en sökväg som flyr roten och en fil som hunnit bli en symlänk efter att namnet kontrollerades. Källtyper vars proveniens ingen adapter kan fastställa avvisas.
>
> Vad #18 (PR D) **inte** täcker: `verified` och `contradicted` är båda onåbara därifrån — ett oanvändbart citat säger att pekaren är trasig, aldrig att påståendet är falskt. Semantisk verifiering är PR E. `stale CI` ur ruta fyra saknar fortfarande en `ci`-observationskälla att kontrollera mot. Ett fynd kan bara citera det som observerats: ett korrekt citat av rader bortom utdraget avvisas i stället för att läsas in, vilket betyder att en bredare citering kräver en observation som täcker raderna.
