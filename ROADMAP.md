...
- [ ] Sätt explicit exit criteria per task i stället för generell textlängd/rubriker. Ingen finding med okänd täckning får tilldelas verifierad severity.
  ...
  - **Kriterier ur frågan finns nu.** När planen avgränsat till ett ämne läggs `question_addressed` till, och dess kravtext *är* frågan ordagrant. Det uppfylls när minst ett fynd avgjorts **till sin fördel** mot en källa planen namngav. `contradicted` räcker inte: en vederläggning säger att producenten hade fel, inte att frågan avgjorts av det den skrev — den som vill fastställa ett negativt påstående formulerar `source_lacks_literal`, som ett `verified`-utslag sedan bär.
  ...
  - **En granskning som påstår ingenting passerar inte längre en avgränsad granskning.** Den uppfyller varje kriterium om vad fynd är värda — det finns inga att vara värda något — och den har inte besvarat det den fick i uppdrag. Det stänger vid grinden det som slutraden bara kunde varna för.
  - Ny nästa åtgärd `answer_the_question`, producentens, rankad efter läsning och före allt om hur bra fynden är: ett svar om fel ämne går inte att reparera till ett svar om rätt ämne.
  - **Verklig omfattning:** kriteriet prövar *relevans*, inte fullständighet. Ett avgjort påstående om en källa planen namngav räcker, även om det inte är det påstående frågan egentligen efterfrågar — en körning kan fastställa något trivialt om `settings.env` och passera utan att ha svarat på om ett lösenord är committat. Frågan i sig kommer dessutom ur nyckelordsmatchning (ruta ett), så "den konkreta frågan" är en av fem deklarerade, inte en formulerad för uppgiften.
  - **Kryssas vid merge**, enligt stängningsregeln. Klartkriteriet är prövat i `tests/test_question_criteria.py`: ett långt, välformaterat, citerat och verifierat svar om fel fil faller på `question_addressed`, medan ett 705 tecken kort svar med ett avgjort påstående om rätt källa passerar.
