...
- [ ] Inför en begränsad `repo_review`-plan: välj snapshot → identifiera konkret fråga → läs relevanta filer/tester/CI → samla fynd → verifiera → föreslå nästa *skrivskyddade* undersökning eller avsluta.
  ...
  - **Återstår:** själva planformen. `build_plan` returnerar fortfarande en statisk lista stegnamn; "välj snapshot" och "identifiera konkret fråga" görs av anroparen, inte av planen, och planen väljer inte vilka filer, tester eller CI-körningar som är relevanta för frågan. Rutan stängs när planen gör det.
