...
- [x] Versionera explicit state machine och stopporsaker: `passed`, `insufficient_evidence`, `blocked`, `approval_required`, `budget_exhausted`, `max_iterations`, `no_progress`, `tool_error`, `cancelled`. Skilj runtime-fel från saklig evaluering.
  - **Verifierad i #24 (`821a750`): 355 tester gröna på `main` i testworkflow `35531911033`.** `atlas_core/machine.py` och `schemas/atlas-state-machine.v1.json` gör livscykeln till en tabell: tillstånd, tillåtna övergångar, nio stopporsaker och per orsak dess terminaltillstånd, klass och exitkod. Tabellen är enda källan — `state.stop()` skriver båda fälten ur samma post, så ett runtime-fel inte kan påstå sig vara klart, och `enter()` vägrar en övergång tabellen inte tillåter, inklusive en andra terminalövergång.
  - Uppdelningen runtime/saklig ligger i `stop_class`, härledd ur `stop_reason` och inte lagrad bredvid den. `failed` blev `tool_error`; `no_actionable_retry` delades i `insufficient_evidence`, `blocked` och `no_progress`, som bad läsaren om motsatta saker under ett namn. `blocked` avgörs ur `AtlasEvaluation.blocked_by` — statuskoder, inte den prosa evaluatorn redan skrivit.
  - Rättat samtidigt: iterationsgränsen rapporteras nu bara när den faktiskt band något. Den tidigare kontrollern valde `max_iterations` utifrån saknade rubriker, så en körning som förbrukade alla varv på ett evidensgap rapporterade i stället att den inte hade något kvar att pröva.
  - **Senare täckning:** `budget_exhausted` produceras nu i budgeterade körningar efter #26 (`22e0990`). `cancelled` är alltjämt deklarerad men inte nåbar; avbrott hör till ruta fyra. `no_progress` är här a priori-formen: evaluatorn fann inget en ny omgång kunde ändra. Att upptäcka att två varv gav *samma* output hör fortfarande till P1.1.
...
**P0 exit gate (fortfarande öppen tills P0.3 ruta två–fyra och säkerhetsgränserna är verifierade):** en osann verifierbar claim kan inte få `passed`; varje beslut kan följas till snapshot/evidence; read-only-försök att skriva nekas på exekveringsgränsen, inte bara genom prompttext.
