...
- [ ] Ge feedback som strukturerade gapkoder och `next_action`, inte bara fri prosa. Kräv ny observation, nytt test eller förändrad plan före retry; identisk körning stoppas `no_progress`.
  ...
  - **Återstår:** "kräv **ny observation**, nytt test eller förändrad plan före retry" i bokstavlig mening. Loopen kan i dag inte skaffa en ny observation mitt i en körning — observationerna är fasta när körningen startar — så villkoret är uppfyllt i den svagare formen "felet måste ha rört sig". Den starkare formen hör ihop med ruta ett, som äger läs-steget i `repo_review`-planen.
