...
class TestTheBoundIsOnlyTheReasonWhenItBound(unittest.TestCase):
    ...
    def _uncited(self, max_iterations: int) -> dict:
        ...
        Box four stops a run whose feedback repeats, budgeted or not, so a
        producer that emits the same failure twice no longer gets to the
        bound — it stops on `no_progress`, which is a more specific true thing
        about it. This fixture therefore fixes the missing section on the
        second pass and leaves the evidence gap standing: still failing, still
        something another pass could act on, and now genuinely stopped by the
        iteration count rather than by repeating itself.
        """
        body = (
            "# Repo Review\n\n## Observed sources\n- README.md\n\n"
