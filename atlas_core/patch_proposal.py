    ...
    try:
        ...
        if log is not None:
            log.append("write_verified", verified=verified,
                       checks=[{k: c[k] for k in ("name", "passed")} for c in checks],
...
def side_effects(proposal: Proposal, *, rolled_back: bool) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"what": "fetched the proposal's objects into the repository", "reversible": True,
         "undo": "unreachable once the branch is gone; git gc prunes them"},
