...
def collect_observation_safely(
    ...
    """
    resolve_within(snapshot.root, relative_path)
    return collect_observation(snapshot, relative_path, **kwargs)
...
def redacted_manifest(
    ...
    *,
    ...
    Excerpts are masked; `content_sha256` is not, because the manifest has to
    keep pointing at what was actually verified. When `root` is given, each
    entry is re-verified so the manifest states whether the source still backs
    it rather than leaving a reader to assume it does.
    """
    entries: list[dict[str, Any]] = []
    for observation in observations:
        entry: dict[str, Any] = {
            **_with_redacted_excerpt(observation).to_dict(),
...
def _with_redacted_excerpt(observation: Observation) -> Observation:
    """Return the observation with only its excerpt masked.

    Built through `Observation.create` so the result is validated like any
    other, and so `content_sha256` — computed over raw content elsewhere — is
    carried across untouched.
    """
    redacted = redact_text(observation.excerpt)
    if redacted == observation.excerpt:
        return observation
